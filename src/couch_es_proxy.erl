...
-export([handle_proxy_req/2, handle_proxy_req/3, handle_proxy_req/4]).
...
handle_proxy_req(Req, Url) ->
    handle_proxy_req(Req, Url, get_body(Req), get_headers(Req)).

handle_proxy_req(Req, Url, Body) ->
    handle_proxy_req(Req, Url, Body, get_headers(Req)).

handle_proxy_req(Req, Url, Body, Headers) ->
    %% Bug in Mochiweb?
    ...
    Headers1 = to_ibrowse_headers(Headers, []),
    Options = [
        {http_vsn, Version},
        {headers_as_is, true},
    ...
    case ibrowse:send_req(Url, Headers1, Method, Body, Options, ?TIMEOUT) of
        {ibrowse_req_id, ReqId} ->
            stream_response(Req, Url, ReqId);
        {error, Reason} ->
            throw({error, Reason})
    end.
...
get_urls(Req, ProxyDest) ->
    SourceUrl = couch_httpd:absolute_uri(Req, couch_httpd:path(Req)),
